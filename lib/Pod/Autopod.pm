...
#  $this->foo($text);
#
# The object "$this" is the default and automatially used when a constructor was found ("new"). You
# can change this by the parameter "selfstring" in the autoperl constructor.
#
...
my %v=@_; 
	...
	if ($this->{'readfile'}){
		my $arr = $this->_getFileArray($this->{'readfile'});
...
my $arr=shift or die "Arrayref expected";	
	...
	for (my $i=0;$i < scalar(@$arr); $i++){
		...
		if ((($line=~ m/^\s*\}/) || ($p == 0)) && ($this->{'STATE'}=~ m/^(head|headwait|bodywait|free)$/)){ ## last line of body
	...
	if (exists $this->{'METHOD_ATTR'}->{'new'}){ ## its a class!
		$this->{'ISCLASS'}=1;
	}
...
my $this=shift;
	...
	my %dontshow;
	my @dontshow = qw(vars strict warnings libs);
...
my $this=shift;
	...
	my @first = qw(NAME SYNOPSIS DESCRIPTION REQUIRES EXPORTS HOWTO NOTES METHODS);
